export const TAB_HEIGHT = 30;

//// NEUTRALS
export const CREAM = "#fefbea";
export const TRANSPARENT_CREAM = "rgba(254, 251, 234, 1)";
export const LIGHT_BEIGE = "#fae2c0"
export const DARK_BEIGE = "#ae6e17";
export const BURGUNDY = "#390617"
export const MID_BURGUNDY = "#6b2338";
export const LIGHT_BURGUNDY = "#82304b";
export const CHOCOLATE_COLOR = "#500F0A"

//// BLUES
export const PALE_BLUE = "#c8e1f5";
export const LAVENDER = "#e6d9f5";
export const CLOUDY_SKY_COLOR = "#A9C1C9"
export const SKY_COLOR = "#b7d8fe";
export const MID_BLUE = "#4070c2";
export const CORNFLOWER_BLUE = "#2d76f7";
export const BRIGHT_BLUE = "#60a2ee";
export const DARK_BLUE = "#172741";
export const BLUE_CLEAR = "#20bcef";

//// GREENS
export const CHARTREUSE = "#bfc70f";
export const LIGHT_GREEN = "#dcd7c4"
export const BRIGHT_GREEN = "#afaf29";
export const DARK_GREEN = "#646225";
export const GREEN_AVOCADO = "#d1c260";
export const GREEN_MUSTARD = "#c28d1c";
export const GREEN_KELLY = "#06923b";

//// WARM TONES
export const ORANGE = "#ea5831";
export const PEACH = "#fcd1cd";
export const BRIGHT_ORANGE = "f94e19";
export const PINK_BUBBLEGUM = "#ff96b5";





export const BOLD_BLUE = "#5da7e6";
export const BOLD_GREEN = "#d9e811";
export const BOLD_PINK = "#f585ab";
export const BOLD_ORANGE = "#e67107";
export const BOLD_BROWN = "#431515";

export const APP_BACKGROUND_COLOR = CREAM;
export const APP_HEADER_TEXT_COLOR = BOLD_BROWN;